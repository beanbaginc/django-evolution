#!/usr/bin/env python
#
# Performs a release of Django-Evolution. This can only be run by the core
...
from django_evolution import __version__, __version_info__, is_release
...
BUG_URL = 'https://code.google.com/p/django-evolution/issues/detail?id='
RELEASES_BUCKET_NAME = 'downloads.reviewboard.org'
RELEASES_BUCKET_KEY = '/releases/%s/%s.%s/' % (PACKAGE_NAME.replace('_', '-'),
                                               __version_info__[0],
                                               __version_info__[1])
